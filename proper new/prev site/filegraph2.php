...
$querytop = "SELECT  `ISBN` , `year`, SUM(  `quantity sold` ) AS  `Total Quantity` FROM  `psales` GROUP BY  `ISBN` 
...
$mshandle = $connection->query("SELECT `ISBN`,  SUM(`quantity sold`) AS 'Total Quantity' FROM `psales` WHERE `ISBN` = {$risbn} AND `month` = '{$monthn}' AND  `year` = {$cyear} GROUP BY `ISBN`");
...
$msales1 = array();
// Select all months for sales year & put in array
$monthsnh = $connection->query("SELECT DISTINCT `month` FROM `psales` WHERE `ISBN` = '{$risbn}'");
while ($month = $monthsnh->fetch_assoc()){
	$marray1[] = $month['month'];
}
...
// start logic of gathering monthly sales
foreach($marray1 as $monthn){
$mshandle = $connection->query("SELECT `ISBN`,  SUM(`quantity sold`) AS 'Total Quantity' FROM `psales` WHERE `ISBN` = '{$risbn}' AND `month` = '{$monthn}' GROUP BY `ISBN`");
...
$msales1 = array();
// Select all months for sales year & put in array
$monthsnh = $connection->query("SELECT DISTINCT `month` FROM `psales`, `products` WHERE `psales`.`ISBN` = `products`.`ISBN` 
AND `code` = '{$rcode}'");
while ($month = $monthsnh->fetch_assoc()){
	$marray1[] = $month['month'];
}
...
// start logic of gathering monthly sales
foreach($marray1 as $monthn){
$mshandle = $connection->query("SELECT `psales`.`ISBN`,  SUM(`quantity sold`) AS 'Total Quantity' FROM `psales`, `products` WHERE 
`psales`.`ISBN` = `products`.`ISBN` 
AND `code` = '{$rcode}' 
