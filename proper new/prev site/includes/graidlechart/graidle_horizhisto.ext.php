...
{
	protected function drawHorizHisto()
	{
		for($m=$t=0;$t<count($this->value);$t++)
		{
			if($this->type[$t]=='hb')
			{
				...
				$cc=imagecolorallocatealpha($this->im,$red,$green,$blue,15);
				$cb=imagecolorallocatealpha($this->im,(int)($red/2),(int)($green/2),(int)($blue/2),75);
				$strExtVal=NULL;

				for($y=$this->a+($this->larg/2),$valuelen=2,$i=0;$i<count($this->value[$t]);$i++,$y+=$this->ld,$valuelen=2)
				{
					$y2=$y+((($this->larg+1)-($this->BarOffset*$this->larg))*$m);
					$y1=$y2+$this->larg;
					...
					if(isset($this->multicolor)&&$this->multicolor==1){
						$cc=next($this->colours);
						list($name,$red,$green,$blue)=explode(',',$cc);
						$cc=imagecolorallocatealpha($this->im,$red,$green,$blue,25);
						$cb=imagecolorallocatealpha($this->im,(int)($red/2),(int)($green/2),(int)($blue/2),50);
					}
				}
			}
			$m++;
	...
	public function gradAxis($sy=NULL,$sx=NULL)
	{
		...
		{
			$x1=$x-(int)((count($n)*$this->font_small/2));
		...
		{
			$x1=$x-(int)((graidle::stringLen($n)*$this->font_small)/2);
