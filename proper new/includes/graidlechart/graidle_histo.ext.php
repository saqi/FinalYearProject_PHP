	...
	protected function drawHisto()
	{
		for($m=$t=0;$t<count($this->value);$t++)
		{
			if($this->type[$t]=='b')
			{
				...
				$cb=imagecolorallocatealpha($this->im,round($red/2),round($green/2),round($blue/2),75);
		    $red=	imagecolorallocatealpha($this->im,250,10,80,0);

				for($x=$i=0 ; $i<count($this->value[$t]) ; $i++){
					if($x==0){
						$x1= $this->s + (($this->larg+1)-($this->BarOffset*$this->larg))*$m + ( $this->larg/2 );
						$x2=$x1+$this->larg;
						$y1=$this->h-$this->b;
						$y2=$this->h-$this->b;
            ...
            for($sum=$s=0;$s<count($this->value[$t]);$s++)
						...
						switch($this->ExtLeg){
							...
							case 1: $strExtVal=round(($this->value[$t][$i]/$sum)*100,1)."% ";break;
								...
								imagettftext($this->im,$this->font_small,0,$x1+$this->larg/10,$y1-4,$red,$this->font,$strExtVal);
					}
				}
			$m++;
			}
